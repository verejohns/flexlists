...
const RenderField = ({
  ...
  const setDateValue = (columnId: number, date: Dayjs | null) => {
    try {
      ...
      setValues({ ...values, [columnId]: dateString });
    } catch (e) {}
  };
  ...
  const setTimeValue = (columnId: number, time: Dayjs | null) => {
    ...
    setValues({
      ...values,
      [columnId]: time.format(utcFormat),
    });
  };
  ...
  switch (column.uiField) {
    case FieldUiTypeEnum.Text:
      return currentMode !== "view" && !isPrint ? (
        ...
        <tr key={column.id}>
          ...
          <td>{values ? values[column.id]?.toString() : ""}</td>
          {/* <TextField
            fullWidth
            InputProps={{
              readOnly: true,
    ...
    case FieldUiTypeEnum.LongText:
      return currentMode !== "view" && !isPrint ? (
        ...
        <tr key={column.id}>
          ...
          <td>
            <pre>{values ? values[column.id]?.toString() : ""}</pre>
          </td>
          {/* <TextField
            fullWidth
            multiline
    ...
    case FieldUiTypeEnum.Money:
      return currentMode !== "view" && !isPrint ? (
        ...
        <tr key={column.id}>
          <td>
            <strong>{column.system ? t(column.name) : column.name}</strong>
          </td>
          <td>
      ...
      if (currentMode !== "view" && !isPrint) {
        ...
        return (
          <tr key={column.id}>
            ...
            <td>{choice?.map((x) => x.label)?.join(", ")}</td>
          </tr>
          // <div key={column.id}>
          //   <TextField
          //     fullWidth
        ...
        <tr key={column.id}>
          ...
          <td>
            <MarkdownEditor
              ...
              isPrint={currentMode === "view" || isPrint}
            />
          </td>
        </tr>
      );
          ...
          {values && values[column.id] !== null && (
            <ViewUserSelect
              ...
              setSelectedUserData={(userData: string) => {
                setValues({ ...values, [column.id]: userData });
              }}
              name={column.name}
              label={column.system ? t(column.name) : column.name}
