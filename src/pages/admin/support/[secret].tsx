...
const SupportDetail = ({
  ...
  const ADMIN_TICKET_MESSAGE_INTERVAL = parseInt(
    process.env.NEXT_PUBLIC_FLEXLIST_ADMIN_TICKET_MESSAGE_INTERVAL || "1"
  );
  ...
  useEffect(() => {
    ...
    if (
      ...
      }, ADMIN_TICKET_MESSAGE_INTERVAL * 1000);
  ...
  }, [router.isReady, userProfile, router.query.secret, authValidate]);
  ...
  const handleSubmit = async () => {
    ...
    const createTicketMessageResponse =
      await supportService.createStaffTicketMessage(
        router.query.secret as string,
        newMessage,
        status
      );
    ...
    if (isSucc(createTicketMessageResponse)) {
      ...
      setFlashMessage({
        message: (createTicketMessageResponse as FlexlistsError)?.message,
        type: "error",
      });
      return;
    }
